...
public class RefreshTokenInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        Map<String, String> userMap =
                stringRedisTemplate.<String, String>opsForHash().entries(LoginConst.USER_INFO + token);
        // 判断用户是否存在
        if (userMap.isEmpty()) {
            return true;
        }
        ...
        String redisToken =
                stringRedisTemplate.opsForValue().get(LoginConst.USER_TOKEN + info.getTenantId() + ":" + info.getId());
        // 缓存中登录用户的token和当前请求客户端的token不一致，删除当前请求客户端token关联的用户数据
        if (!Objects.equals(redisToken, token)) {
            // 删除缓存用户信息
            stringRedisTemplate.delete(LoginConst.USER_INFO + token);
            return true;
        }
        // 刷新用户信息
        stringRedisTemplate.expire(LoginConst.USER_INFO + token,
                loginConstants.getExpireTimeLogin(), TimeUnit.SECONDS);
        stringRedisTemplate.expire(LoginConst.USER_TOKEN + info.getTenantId() + ":" + info.getId(), loginConstants.getExpireTimeLogin(), TimeUnit.SECONDS);
        return true;
    }
}
