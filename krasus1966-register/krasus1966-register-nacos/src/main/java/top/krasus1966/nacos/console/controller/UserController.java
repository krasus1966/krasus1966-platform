...
import top.krasus1966.nacos.console.security.nacos.JwtTokenManager;
import top.krasus1966.nacos.console.security.nacos.NacosAuthConfig;
import top.krasus1966.nacos.console.security.nacos.NacosAuthManager;
...
public class UserController {
     ...
     * @since 1.2.0
     */
    @Secured(resource = NacosAuthConfig.CONSOLE_RESOURCE_NAME_PREFIX + "users", action =
            ActionTypes.WRITE)
    @PostMapping
    ...
    public RestResult<String> updatePassword(@RequestParam(value = "oldPassword") String oldPassword,
        ...
        try {
            if (PasswordEncoderUtil.matches(oldPassword, password)) {
                userDetailsService.updateUserPassword(username,
                        PasswordEncoderUtil.encode(newPassword));
                rr.setCode(200);
